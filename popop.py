...
types_en = ['rili', 'dongman', 'fengjing', 'meinv', 'youxi', 'yingshi', 'dongtai', 'weimei', 'sheji', 'keai', 'qiche', 'huahui', 'dongwu', 'jieri', 'renwu', 'meishi', 'shuiguo', 'jianzhu', 'tiyu', 'junshi', 'feizhuliu', 'qita', 's/wangzherongyao', 's/huyan', 's/lol','']
...
class myThread(threading.Thread):  # 继承父类threading.Thread
    ...
    def run(self):  # 把要执行的代码写到run函数里面 线程在创建后会直接运行run函数
        ...
        for url in self.urls1:
            browser.get(url)
            m = browser.find_elements_by_xpath('//div[@id="main"]/div[@class="endpage"]/div[@class="pic"]/p/a')
            href2 = m[0].get_attribute('href')
            browser.get(href2)
            urll = browser.find_elements_by_xpath('//*[@id="endimg"]/tbody/tr/td/a')
            href3 = urll[0].get_attribute('href')
            img = requests.get(href3, headers=headers).content
            cha = r'~`!@#$%^&()-={}:;'
            path = "C:\\Users\\86132\\Desktop\\images\\img" + self.name + str(self.n) + str(random.randint(0, 100)) + random.choice(cha) + ".jpg"
...
def ts(num, low, urls1):
    nn = 1
    urls1a = urls1[0:num]
    urls1b = urls1[num:num*2]
    urls1c = urls1[num*2:num*3]
    urls1d = urls1[num*3:num*4+low]
    # 创建新线程
